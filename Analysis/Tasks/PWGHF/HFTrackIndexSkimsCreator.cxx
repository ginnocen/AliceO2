...
struct SelectTracks {
  ...
  Configurable<double> dcatoprimxy_2prong_minpt{"dcatoprimxymin_2prong_minpt", 2., "max pt cut for min. DCAXY to prim. vtx."};
  Configurable<double> dcatoprimxymin_2prong{"dcatoprimxymin_2prong", 0., "min. DCAXY to prim. vtx."};
  Configurable<double> dcatoprimxymax_2prong{"dcatoprimxymax_2prong", 1.0, "max. DCAXY to prim. vtx."};
  ...
  Configurable<double> dcatoprimxy_3prong_minpt{"dcatoprimxy_3prong_minpt", 2., "max pt cut for min. DCAXY to prim. vtx."};
  Configurable<double> dcatoprimxymin_3prong{"dcatoprimxymin_3prong", 0., "min. DCAXY to prim. vtx."};
  Configurable<double> dcatoprimxymax_3prong{"dcatoprimxymax_3prong", 1.0, "max. DCAXY to prim. vtx."};
  ...
  HistogramRegistry registry{
    ...
    {{"hpt_nocuts", "all tracks;#it{p}_{T}^{track} (GeV/#it{c});entries", {HistType::kTH1F, {{100, 0., 10.}}}},
     ...
     {"hdcatoprimxy_cuts_2prong", "tracks selected for 2-prong vertexing;DCAxy to prim. vtx. (cm);entries", {HistType::kTH1F, {{100, -1., 1.}}}},
     {"heta_cuts_2prong", "tracks selected for 2-prong vertexing;#it{#eta};entries", {HistType::kTH1F, {{100, -1., 1.}}}},
     // 3-prong histograms
     {"hpt_cuts_3prong", "tracks selected for 3-prong vertexing;#it{p}_{T}^{track} (GeV/#it{c});entries", {HistType::kTH1F, {{100, 0., 10.}}}},
     {"hdcatoprimxy_cuts_3prong", "tracks selected for 3-prong vertexing;DCAxy to prim. vtx. (cm);entries", {HistType::kTH1F, {{100, -1., 1.}}}},
     {"heta_cuts_3prong", "tracks selected for 3-prong vertexing;#it{#eta};entries", {HistType::kTH1F, {{100, -1., 1.}}}}} //
  };
               ...
               soa::Join<aod::Tracks, aod::TracksCov, aod::TracksExtra> const& tracks)
  {
    ...
    for (auto& track : tracks) {
      ...
      array<float, 2> dca;
      if (status_prong > 0) {
        double dcatoprimxymin_2prong_ptdep = dcatoprimxymin_2prong * TMath::Max(0., (1 - TMath::Floor(trackPt / dcatoprimxy_2prong_minpt)));
        double dcatoprimxymin_3prong_ptdep = dcatoprimxymin_3prong * TMath::Max(0., (1 - TMath::Floor(trackPt / dcatoprimxy_3prong_minpt)));
...
struct HFTrackIndexSkimsCreator {
  ...
  HistogramRegistry registry{
    ...
    {{"hNTracks", ";# of tracks;entries", {HistType::kTH1F, {{2500, 0., 25000.}}}},
     ...
     {"hNCand2Prong", "2-prong candidates;# of candidates;entries", {HistType::kTH1F, {{2000, 0., 200000.}}}},
     {"hNCand2ProngVsNTracks", "2-prong candidates;# of tracks;# of candidates;entries", {HistType::kTH2F, {{2500, 0., 25000.}, {2000, 0., 200000.}}}},
     ...
     {"hNCand3Prong", "3-prong candidates;# of candidates;entries", {HistType::kTH1F, {{5000, 0., 500000.}}}},
     {"hNCand3ProngVsNTracks", "3-prong candidates;# of tracks;# of candidates;entries", {HistType::kTH2F, {{2500, 0., 25000.}, {5000, 0., 500000.}}}},
     {"hmassDPlus", "D+ candidates;inv. mass (#pi K #pi) (GeV/#it{c}^{2});entries", {HistType::kTH1F, {{500, 1.6, 2.1}}}},
     {"hmassLc", "Lc candidates;inv. mass (p K #pi) (GeV/#it{c}^{2});entries", {HistType::kTH1F, {{500, 2.1, 2.5}}}},
     {"hmassDs", "Ds candidates;inv. mass (K K #pi) (GeV/#it{c}^{2});entries", {HistType::kTH1F, {{500, 1.7, 2.2}}}}}};
    ...
    SelectedTracks const& tracks)
  {
    ...
    */
    ...
    const int n2Prong = 2;                         //number of two prong hadron types
    const int n3Prong = 3;                         //number of three prong hadron types
    int n2ProngBit = TMath::Power(2, n2Prong) - 1; //bit value for 2 prongs when all hadrons are set to true
    int n3ProngBit = TMath::Power(2, n3Prong) - 1; //bit value for 3 prongs when all hadrons are set to true

    //retrieve cuts from json - to be made pT dependent when option appears in json
    double cut2ProngPtCandMin[n2Prong] = {configs->getPtD0Min(), configs->getPtJpsiMin()};
    double cut2ProngInvMassCandMin[n2Prong] = {configs->getInvMassD0Min(), configs->getInvMassJpsiMin()};
    double cut2ProngInvMassCandMax[n2Prong] = {configs->getInvMassD0Max(), configs->getInvMassJpsiMax()};
    double cut2ProngCPACandMin[n2Prong] = {configs->getCPAD0Min(), configs->getCPAJpsiMin()};
    double cut2ProngImpParProductCandMax[n2Prong] = {configs->getImpParProductD0Max(), configs->getImpParProductJpsiMax()};

    double cut3ProngPtCandMin[n3Prong] = {configs->getPtDPlusMin(), configs->getPtLcMin(), configs->getPtDsMin()};
    double cut3ProngInvMassCandMin[n3Prong] = {configs->getInvMassDPlusMin(), configs->getInvMassLcMin(), configs->getInvMassDsMin()};
    double cut3ProngInvMassCandMax[n3Prong] = {configs->getInvMassDPlusMax(), configs->getInvMassLcMax(), configs->getInvMassDsMax()};
    double cut3ProngCPACandMin[n3Prong] = {configs->getCPADPlusMin(), configs->getCPALcMin(), configs->getCPADsMin()};
    double cut3ProngDecLenCandMin[n3Prong] = {configs->getDecLenDPlusMin(), configs->getDecLenLcMin(), configs->getDecLenDsMin()};
    ...
    double mass2Prong1[n2Prong];
    double mass2Prong2[n2Prong];

    double mass3Prong1[n3Prong];
    double mass3Prong2[n3Prong];
    ...
    for (auto trackPos1 = tracks.begin(); trackPos1 != tracks.end(); ++trackPos1) {
      ...
      for (auto trackNeg1 = tracks.begin(); trackNeg1 != tracks.end(); ++trackNeg1) {
        ...
        if (sel2ProngStatus > 0) {
          ...
          for (int n2 = 0; n2 < n2Prong; n2++) {
            mass2Prong1[n2] = RecoDecay::M(arrMom, arr2Mass1[n2]);
            mass2Prong2[n2] = RecoDecay::M(arrMom, arr2Mass2[n2]);
            if ((isSelected2ProngCand & 1 << n2) && cut2ProngInvMassCandMin[n2] >= 0. && cut2ProngInvMassCandMax[n2] > 0.) {
              if ((mass2Prong1[n2] < cut2ProngInvMassCandMin[n2] || mass2Prong1[n2] >= cut2ProngInvMassCandMax[n2]) &&
                  (mass2Prong2[n2] < cut2ProngInvMassCandMin[n2] || mass2Prong2[n2] >= cut2ProngInvMassCandMax[n2])) {
          ...
          if (isSelected2ProngCand > 0 && df2.process(trackParVarPos1, trackParVarNeg1) > 0) { //should it be this or > 0 or are they equivalent
            ...
            // candidate pT cut
            if (std::count_if(std::begin(cut2ProngPtCandMin), std::end(cut2ProngPtCandMin), [](double d) { return d >= 0.; }) > 0) {
              double cand2ProngPt = RecoDecay::Pt(pVecCandProng2);
              for (int n2 = 0; n2 < n2Prong; n2++) {
                if (cand2ProngPt < cut2ProngPtCandMin[n2]) {
                  isSelected2ProngCand = isSelected2ProngCand & ~(1 << n2);
                }
              }
            }
            ...
            // imp. par. product cut
            if (std::count_if(std::begin(cut2ProngImpParProductCandMax), std::end(cut2ProngImpParProductCandMax), [](double d) { return d < 100.; }) > 0) {
              auto impParProduct = trackPos1.dcaPrim0() * trackNeg1.dcaPrim0();
              for (int n2 = 0; n2 < n2Prong; n2++) {
                if ((isSelected2ProngCand & 1 << n2) && impParProduct > cut2ProngImpParProductCandMax[n2]) {
                  isSelected2ProngCand = isSelected2ProngCand & ~(1 << n2);
                }
              }
            }
            ...
            // CPA cut
            if (isSelected2ProngCand > 0 && std::count_if(std::begin(cut2ProngCPACandMin), std::end(cut2ProngCPACandMin), [](double d) { return d > -2.; }) > 0) {
              auto cpa = RecoDecay::CPA(array{collision.posX(), collision.posY(), collision.posZ()}, secondaryVertex2, pVecCandProng2);
              for (int n2 = 0; n2 < n2Prong; n2++) {
                if ((isSelected2ProngCand & 1 << n2) && cpa < cut2ProngCPACandMin[n2]) {
                  isSelected2ProngCand = isSelected2ProngCand & ~(1 << n2);
                }
              }
            }
              ...
              if (b_dovalplots) {
                ...
                for (int n2 = 0; n2 < n2Prong; n2++) {
                  if (isSelected2ProngCand & 1 << n2) {
                    if ((cut2ProngInvMassCandMin[n2] < 0. && cut2ProngInvMassCandMax[n2] <= 0.) || (mass2Prong1[n2] >= cut2ProngInvMassCandMin[n2] && mass2Prong1[n2] < cut2ProngInvMassCandMax[n2])) {
                      mass2Prong1[n2] = RecoDecay::M(arrMom, arr2Mass1[n2]);
                      if (n2 == 0) {
                        registry.get<TH1>("hmassD0")->Fill(mass2Prong1[n2]);
                      }
                      if (n2 == 1) {
                        registry.get<TH1>("hmassJpsi")->Fill(mass2Prong1[n2]);
                      }
                    }
                    if ((cut2ProngInvMassCandMin[n2] < 0. && cut2ProngInvMassCandMax[n2] <= 0.) || (mass2Prong2[n2] >= cut2ProngInvMassCandMin[n2] && mass2Prong2[n2] < cut2ProngInvMassCandMax[n2])) {
                      mass2Prong2[n2] = RecoDecay::M(arrMom, arr2Mass2[n2]);
                      if (n2 == 0) {
                        registry.get<TH1>("hmassD0")->Fill(mass2Prong1[n2]);
                      }
        ...
        if (do3prong == 1) {
          ...
          for (auto trackPos2 = trackPos1 + 1; trackPos2 != tracks.end(); ++trackPos2) {
            ...
            for (int n3 = 0; n3 < n3Prong; n3++) {
              mass3Prong1[n3] = RecoDecay::M(arr3Mom, arr3Mass1[n3]);
              mass3Prong2[n3] = RecoDecay::M(arr3Mom, arr3Mass2[n3]);
              if ((isSelected3ProngCand & 1 << n3) && cut3ProngInvMassCandMin[n3] >= 0. && cut3ProngInvMassCandMax[n3] > 0.) {
                if ((mass3Prong1[n3] < cut3ProngInvMassCandMin[n3] || mass3Prong1[n3] >= cut3ProngInvMassCandMax[n3]) &&
                    (mass3Prong2[n3] < cut3ProngInvMassCandMin[n3] || mass3Prong2[n3] >= cut3ProngInvMassCandMax[n3])) {
            ...
            if (b_dovalplots) {
              ...
              for (int n3 = 0; n3 < n3Prong; n3++) {
                if (isSelected3ProngCand & 1 << n3) {
                  if ((cut3ProngInvMassCandMin[n3] < 0. && cut3ProngInvMassCandMax[n3] <= 0.) || (mass3Prong1[n3] >= cut3ProngInvMassCandMin[n3] && mass3Prong1[n3] < cut3ProngInvMassCandMax[n3])) {
                    mass3Prong1[n3] = RecoDecay::M(arr3Mom, arr3Mass1[n3]);
                    if (n3 == 0) {
                      registry.get<TH1>("hmassDPlus")->Fill(mass3Prong1[n3]);
                    }
                    if (n3 == 1) {
                      registry.get<TH1>("hmassLc")->Fill(mass3Prong1[n3]);
                    }
                    if (n3 == 2) {
                      registry.get<TH1>("hmassDs")->Fill(mass3Prong1[n3]);
                    }
                  }
                  if ((cut3ProngInvMassCandMin[n3] < 0. && cut3ProngInvMassCandMax[n3] <= 0.) || (mass3Prong2[n3] >= cut3ProngInvMassCandMin[n3] && mass3Prong2[n3] < cut3ProngInvMassCandMax[n3])) {
                    mass3Prong2[n3] = RecoDecay::M(arr3Mom, arr3Mass2[n3]);
                    if (n3 == 1) {
                      registry.get<TH1>("hmassLc")->Fill(mass3Prong2[n3]);
                    }
                    if (n3 == 2) {
                      registry.get<TH1>("hmassDs")->Fill(mass3Prong2[n3]);
                    }
                  }
                }
              }
