  ...
  { /*
    ...
    for (auto& candidate : candidates) {
      ...
      if (std::abs(candidate.flagMCMatchRec()) == 1 << DecayType::XiccToXicPi) {
        ...
        auto indexMother = RecoDecay::getMother(particlesMC, candidate.index0_as<aod::BigTracksMC>().mcParticle_as<soa::Join<aod::McParticles, aod::HfCandXiccMCGen>>(), pdg::Code::kXiCCPlusPlus, true);
